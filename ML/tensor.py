...
df = pd.read_excel('BankruptcyStock.xls',keep_default_na=False, na_values=['#N/A'])
...
for i in range(0,1):
    ...
    summary_writer = tf.summary.FileWriter('C:\\path\\to\\log-directory',sess.graph)
    ...
    for j in range(0,len(x_dataTest[0][0])):
        ...
        if ((sess.run(hypothesis,feed_dict={X: Testlist}) > 0.7)[0][0] == y_dataTest[j]):
            score = score + 1
