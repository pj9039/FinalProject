...
File_List = glob.glob('StockData(NoBankruptcy)/*.xls') # StockData 폴더에 있는 모든 엑셀 데이터불러오기
...
for filelist in File_List:
    ...
    while i < nlow1:
        ...
        i += 1
    try:
        sheet.write(cnt, 7, "0") # StockData폴더에 있는 종목들은 다 상장폐지된 종목이기 때문에 상장폐지가 되었다는 의미로 1를 입력
    except:
        pass
    cnt = cnt+1
wbk.save('NoBankruptcyStock.xls') # 지금까지 쓴 엑셀데이터 저장